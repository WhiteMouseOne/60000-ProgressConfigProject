...
const getPersonalInfo = () => {
  getPersonInfoApi({ employeeNumber: employeeNumber }).then((element: any) => {
    ...
    // 兼容后端 PascalCase
    if (personInfo.value.email == null && element.data.Email != null) {
      personInfo.value.email = element.data.Email
    }
    // 如果返回图片路径在拼接路径
...
const confirmClick = () => {
  personForm.value.validate(async (valid: boolean) => {
    if (valid) {
      ...
      const { id, roleName, userName, newPassword, phoneNumber, email } = personInfo.value
