<template>
    ...
    <el-card class="searchTop" shadow="never">
      <div class="user-header">
        <el-form :inline="true" :model="roleFormInline" class="ef_1">
          <el-form-item :label="$t('systemRole.roleName')+':'">
            <el-input
              v-model="roleFormInline.keywords"
              :placeholder="$t('systemRole.roleNameTip')"
              clearable
              prefix-icon="Search"
              class="ei_1"
              style="width: 190px"
    ...
    <el-card shadow="never">
      ...
      <div class="table" ref="tableContanierRef">
        ...
        >
          ...
          <el-table-column type="index" :label="$t('systemRole.roleId')" align="center" width="100" />
          <el-table-column :label="$t('systemRole.roleNameLabel')" prop="roleName" width="125px" align="center" />
          <el-table-column :label="$t('systemRole.displayOrder')" prop="roleSort" width="125px" align="center" />
          <el-table-column :label="$t('systemRole.createTime')" prop="createTime" align="center" :show-overflow-tooltip="true" />
          <el-table-column :label="$t('systemRole.creator')" :show-overflow-tooltip="true" prop="createBy" align="center" />
          <el-table-column :label="$t('systemRole.editor')" :show-overflow-tooltip="true" prop="updateBy" align="center" />
          <el-table-column :label="$t('systemRole.enable')" prop="enable" align="center" width="100">
            <template #default="{ row }">
              <el-switch
                v-model="row.enable"
                inline-prompt
                inactive-color="#f56c6c"
                :active-text="$t('systemRole.yes')"
                :inactive-text="$t('systemRole.no')"
                @change="changeRoleStatus(row)"
              />
            </template>
          </el-table-column>
          <el-table-column fixed="right" align="center" :label="$t('common.operate')" width="250">
            <template #default="scope">
              ...
              <el-button
                ...
                v-hasPermi="['system:role:edit']"
                >{{ $t('systemRole.menuPermission') }}</el-button
              >
            </template>
            <!-- <template #default="scope">
              <el-button
                ...
                type="primary"
                icon="Edit"
                v-if="scope.row.roleName == 'SuperAdmin'"
                @click="handleEdit(scope.row)"
                v-hasPermi="['system:role:edit']"
                >编辑</el-button
              >
  ...
  <!--菜单权限Dialog -->
  <el-dialog :title="$t('systemRole.permissionAssignment')" v-model="showRoleScope" width="500px" :before-close="handleClose">
    <el-form :model="form" label-width="80px" ref="roleMenuForm">
      <el-form-item label="菜单搜索">
        <el-input :placeholder="$t('systemRole.enterKeyword')" v-model="searchText" />
      </el-form-item>
      <!-- <el-form-item label="权限字符">
          <el-input v-model="form.roleKey" :disabled="true"></el-input>
        </el-form-item> -->
      <el-form-item :label="$t('systemRole.menuPermissionLabel')">
        <el-checkbox v-model="form.menuExpand" @change="handleCheckedTreeExpand($event, 'menu')">{{ $t('systemRole.expandCollapse') }}</el-checkbox>
        <el-checkbox v-model="form.menuNodeAll" @change="handleCheckedTreeNodeAll($event, 'menu')"
          >{{ $t('systemRole.selectAll') }}</el-checkbox
        >
        <el-checkbox v-model="form.menuCheckStrictly" @change="handleCheckedTreeConnect($event, 'menu')"
          >{{ $t('systemRole.parentChildLinkage') }}</el-checkbox
        >
        <el-tree
          class="tree-border"
          :data="menuOptions"
...
const getRoleData = async (config: any) => {
  const res: any = await getRoleDataApi(config)
  if (res) {
...
const changeRoleStatus = (row: any) => {
  const status = row.enable === true ? "启用" : "禁用"
  ElMessageBox.confirm(`确定要 "${status}"  角色 "${row.roleName}" 吗?`, "系统提示", {
    ...
    .then(async () => {
      ...
      const res = await changeRoleEnableApi({
        id: id,
        enable: enable
      })
      if (res) {
...
const handleDelList = () => {
  ElMessageBox.confirm("确定要删除吗", "提示", {
    ...
    .then(async () => {
      if (ids.length !== 0) {
        const res: any = await batchDelRoleApi(ids)
        if (res) {
          ElMessage({
            showClose: true,
            message: res.message,
...
const getRoleMenuTreeselect = async (roleId: any) => {
  const res: any = await getMenuTreeSelectApi(roleId)
  if (res) {
...
const submitRoleScope = async () => {
  if (form.roleId != undefined) {
    ...
    const res: any = await updataRoleMenuScopeApi(params)
    if (res) {
      ElMessage({
        type: "success",
        message: res.message
