    ...
    <el-card shadow="never">
      ...
      <div ref="tableContanierRef" class="table">
        <el-table v-loading="loading" :data="tableData" :fit="true" border :height="tableHeight">
          ...
          <el-table-column prop="code" label="编码" align="center" width="120" /> -->
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="520px" destroy-on-close @closed="resetForm">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="88px">
        ...
        <el-form-item label="名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入工艺名称" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
...
<script setup lang="ts" name="CraftContent">
...
const fetchList = async () => {
  loading.value = true
  try {
    const res: any = await queryCraftAdminApi({
...
const submitForm = async () => {
  ...
  try {
    if (isEdit.value) {
      const res: any = await updateCraftAdminApi(form.id, {
        code: form.code,
        name: form.name.trim()
      })
      ...
      const res: any = await createCraftAdminApi({
        code: form.code,
        name: form.name.trim()
      })
      if ((res as any)?.code === 200 || res) {
...
const handleDelete = (row: { id: number; name: string }) => {
  ElMessageBox.confirm(`确定删除工艺「${row.name}」？`, "提示", { type: "warning" })
    .then(async () => {
      const res: any = await deleteCraftAdminApi(row.id)
      if ((res as any)?.code === 200 || res) {
