...
const initTags = () => {
  ...
  for (const tag of affixTags) {
    console.log("%c [ tag ]-68", "font-size:13px; background:pink; color:#bf2c9f;", tag)
    // 必须含有 name 属性
...
const addTags = (route: RouteLocationNormalizedLoaded) => {
  console.log("%c [ route ]-75", "font-size:13px; background:pink; color:#bf2c9f;", route)
  if (route.name) {
...
watch(visible, (value) => {
  console.log("%c [ value ]-153", "font-size:13px; background:pink; color:#bf2c9f;", value)
